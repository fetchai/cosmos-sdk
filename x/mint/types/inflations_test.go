...
func TestCalculateInflationPerBlockAndIssuance(t *testing.T) {
	...
	tests := []struct {
		...
		{types.NewMunicipalInflation(targetAccounts[0].Address.String(), math.LegacyNewDec(2)), supply.MulRaw(2)},
		// Pass: 1 = 100% inflation
		{types.NewMunicipalInflation(targetAccounts[0].Address.String(), math.LegacyOneDec()), supply},
		// Pass: 0.5 = 50% inflation
		{types.NewMunicipalInflation(targetAccounts[0].Address.String(), math.LegacyNewDecWithPrec(5, 1)), supply.QuoRaw(2)},
		...
		//{types.NewMunicipalInflation(testDenom, targetAccounts[0].Address.String(), math.LegacyNewDecWithPrec(11, 3).Neg()), supply.MulRaw(11).QuoRaw(1000).Neg()},
		//// Pass: -0.5 = -50% inflation
		//{types.NewMunicipalInflation(testDenom, targetAccounts[0].Address.String(), math.LegacyNewDecWithPrec(5, 1).Neg()), supply.QuoRaw(2)},
		//// Pass: -0.999...9 = -99.999...9% inflation
		//{types.NewMunicipalInflation(testDenom, targetAccounts[0].Address.String(), almostOne.Neg()), math.LegacyNewDecFromInt(supply).Mul(almostOne).TruncateInt().Neg()},
	}
